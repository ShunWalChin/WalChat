...
describe('Meta Instagram Login client', () => {
  ...
  it('sanitiza timeout sem expor URL, token ou erro de transporte', async () => {
    const transportError = Object.assign(
      new Error('https://graph.instagram.com/?access_token=segredo'),
      { name: 'TimeoutError' },
    )
    ...
    let caught: unknown
    try {
      await getMetaOwnProfile('segredo')
    } catch (error) {
      caught = error
    }

    expect(caught).toBeInstanceOf(Error)
    expect((caught as Error).message).toContain('HTTP 504')
    expect((caught as Error).message).not.toContain('segredo')
    expect(META_API_TIMEOUT_MS).toBe(15_000)
  })
})
