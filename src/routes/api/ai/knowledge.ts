      ...
      GET: async ({ request }) => {
        try {
          ...
          let query = context.supabase
            ...
            .select(
              'id,ai_agent_id,title,content,source_type,source_url,status,last_used_at,updated_at',
            )
        ...
        try {
          ...
          const { data, error } = await context.supabase
            ...
            .insert({
              ...
              checksum: createHash('sha256').update(body.content).digest('hex'),
            })
            .select('id')
            .single()
          if (error) throw error
      ...
      PATCH: async ({ request }) => {
        try {
          ...
          if (body.sourceType !== undefined)
            changes.source_type = body.sourceType
          if (body.sourceUrl !== undefined) changes.source_url = body.sourceUrl
          if (body.content !== undefined)
            changes.checksum = createHash('sha256')
              .update(body.content)
              .digest('hex')
