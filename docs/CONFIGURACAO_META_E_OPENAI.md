...
- `GET /api/health` retorna `ok: true` e as integrações esperadas como presentes.
- app, Redis, worker e scheduler estão healthy/running.
...
Somente habilite teste ampliado quando A–E estiverem verdes. Live Mode exige adicionalmente App Review/Advanced Access, Human Agent se usado, rate limiting, observabilidade, SMTP, backup/restauração e rotina definitiva de exclusão de dados.
